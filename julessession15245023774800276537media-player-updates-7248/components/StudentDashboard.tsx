          ...
          return (
              <div className="space-y-6 pb-24">
                  ...
                  <div className="space-y-4 mb-6">
                      {settings?.syllabusType === 'DUAL' && (
                          <div className="flex gap-2 mx-1 mb-4">
                              <button 
                                  onClick={() => {
                                  ...
                                  className={`flex-1 py-3 font-bold rounded-xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 ${syllabusMode === 'SCHOOL' ? 'bg-blue-600 text-white ring-2 ring-blue-300' : 'bg-white text-slate-600 border border-slate-200'}`}
                              >
                                  {syllabusMode === 'SCHOOL' && <CheckCircle size={16} />} School Mode
                              </button>
                              <button 
                                  onClick={() => {
                                  ...
                                  className={`flex-1 py-3 font-bold rounded-xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 ${syllabusMode === 'COMPETITION' ? 'bg-purple-600 text-white ring-2 ring-purple-300' : 'bg-white text-slate-600 border border-slate-200'}`}
                              >
                                  {syllabusMode === 'COMPETITION' && <CheckCircle size={16} />} Competition
                              </button>
                          </div>
                      )}
                  ...
                  {/* FEATURES LIST - PROFESSIONAL & STRUCTURED */}
                  <div className="space-y-4">
                      <div className="flex items-center justify-between">
                          <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
                              <List size={18} className="text-blue-600" /> App Features
                          </h3>
                      </div>
                      <div className="space-y-3">
                          {[
                              { id: 1, title: "Video Lectures", desc: "Watch high-quality educational videos with anti-piracy protection. Features custom watermark and blocked share button for security." },
                              { id: 2, title: "PDF Notes", desc: "Access comprehensive PDF study materials. Notes are structured by subject and chapter for easy learning." },
                              { id: 3, title: "MCQ Practice", desc: "Test your knowledge with chapter-wise MCQs. Get instant feedback and track your progress." },
                              { id: 5, title: "NST Coins & Rewards", desc: "Earn coins through study activity and daily logins. Use coins to unlock premium features and external apps." },
                              { id: 6, title: "Dual Syllabus Mode", desc: "Switch between School and Competition modes. Each mode has its own content and professional theme." }
                          ].map((feat) => (
                              <div key={feat.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:border-blue-100 transition-all group">
                                  <div className="flex items-start gap-4">
                                      <div className="w-10 h-10 bg-slate-900 text-white rounded-2xl flex items-center justify-center font-black text-sm shrink-0">
                                          {feat.id}
                                      </div>
                                      <div>
                                          <h4 className="font-black text-slate-800 text-sm mb-1 group-hover:text-blue-600 transition-colors">{feat.title}</h4>
              ...
              <div className="space-y-6 pb-24">
                      ...
                      <div className="bg-white p-1 rounded-xl shadow-sm border border-slate-200 flex gap-2">
                          <button 
                              ...
                              className={`flex-1 py-3 rounded-lg text-xs font-bold transition-all ${syllabusMode === 'SCHOOL' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}
                          >
                              🏫 School Syllabus
                          </button>
                          <button 
                              onClick={() => setSyllabusMode('COMPETITION')}
                              className={`flex-1 py-3 rounded-lg text-xs font-bold transition-all ${syllabusMode === 'COMPETITION' ? 'bg-purple-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}
                          >
                              🏆 Competition
                          </button>
                      </div>
